...
function doSync() {
  if (
    ...
    !companion.permissions.granted("access_calendar")
  ) {
  ...
  calendars
    ...
    .then((calendarIdMap) => {
      ...
      let eventsQuery = {
        startDate: startDate,
        endDate: endDate,
        calendarIds: Object.keys(calendarIdMap),
      };

      return calendars.searchEvents(eventsQuery).then(function (todayEvents) {
        ...
        todayEvents.forEach((event) => {
          let eventIsAllDay =
            event.isAllDay ||
            (event.startDate <= day_start && event.endDate >= day_end);
          if (
            (eventIsAllDay &&
              settings.getItem("timeline_hide_allday") === "true") ||
            (event.userStatus === "declined" &&
              settings.getItem("timeline_hide_declined") === "true") ||
          ...
          deviceEvents.push({
            ...
            isAccepted:
              event.userStatus === "accepted" || event.userStatus === "unknown",
            startDateHour: event.isAllDay
              ? event.startDate.getUTCHours()
              : event.startDate.getHours(),
            startDateMinute: event.isAllDay
              ? event.startDate.getUTCMinutes()
              : event.startDate.getMinutes(),
            endDateHour: event.isAllDay
              ? event.endDate.getUTCHours()
              : event.endDate.getHours(),
            endDateMinute: event.isAllDay
              ? event.endDate.getUTCMinutes()
              : event.endDate.getMinutes(),
            startHours:
              (event.startDate.getTime() +
                (event.isAllDay
                  ? event.startDate.getTimezoneOffset() * 6e4
                  : 0)) /
              3.6e6,
            endHours:
              (event.endDate.getTime() +
                (event.isAllDay
                  ? event.endDate.getTimezoneOffset() * 6e4
                  : 0)) /
              3.6e6,
          });
        });
    ...
    .then((deviceEvents) => {
      ...
      if (
        CURRENT_EVENTS === null ||
        JSON.stringify(deviceEvents) !== JSON.stringify(CURRENT_EVENTS)
      ) {
...
function getCalendarColour(calId, calIdMap) {
  return JSON.parse(
    settings.getItem("cal:" + calIdMap[calId] + ":color") || '"white"'
  );
}

function send_payload_to_device(payload) {
  outbox
    .enqueue("companion_payload", encode(payload))
    .then(function (ft) {
      // Queued successfully
